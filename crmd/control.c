...
/* 		g_hash_table_foreach(ipc_clients, log_connected_client, NULL); */
    ...
    if (te_subsystem->client && te_subsystem->client->ipc) {
        ...
        process_client_disconnect(te_subsystem->client);
    }
    ...
    if (pe_subsystem->client && pe_subsystem->client->ipc) {
        ...
        process_client_disconnect(pe_subsystem->client);
    }
           ...
           enum crmd_fsa_state cur_state, enum crmd_fsa_input current_input, fsa_data_t * msg_data)
{
    ...
    ipcs = mainloop_add_ipc_server(CRM_SYSTEM_CRMD, QB_IPC_SOCKET, &ipc_callbacks);
    if (ipcs == NULL) {
