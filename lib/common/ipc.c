...
crm_ipcs_send(qb_ipcs_connection_t *c, xmlNode *message, gboolean event)
{
    ...
    crm_trace("Sent %s to %p rc=%d: %.120s", type, c, rc, buffer);
    crm_free(buffer);
    return rc;
}
...
crm_ipc_connect(crm_ipc_t *client) 
{
    ...
    if (client->ipc == NULL) {
        crm_perror(LOG_ERR, "Could not establish %s connection", client->name);
        return FALSE;
    }
...
crm_ipc_connected(crm_ipc_t *client) 
{
    if(client == NULL) {
        ...
        crm_trace("fd=%d", client->pfd.fd);
        return FALSE;
    }

    client->pfd.revents = 0;
    poll(&(client->pfd), 1, 0);
    if(client->pfd.revents & (POLLHUP|POLLNVAL)) {
        client->pfd.fd = -1;
        return FALSE;
    }
    return TRUE;
...
crm_ipc_send(crm_ipc_t *client, xmlNode *message, xmlNode **reply, int32_t ms_timeout)
{
    ...
    if(rc == -ENOTCONN) {
        crm_err("Connection to %s failed: %s", client->name);
        client->pfd.fd = -1;
