...
cib_native_signon_raw(cib_t * cib, const char *name, enum cib_conn_type type, int *async_fd, int *unused)
{
    ...
    if (rc != cib_ok || native->ipc == NULL) {
        crm_err("Connection failed");
        rc = cib_connection;
    }
    ...
    if (rc == cib_ok) {
        ...
        if (crm_ipc_send(native->ipc, hello, &reply, 100) > 0) {
...
cib_native_register_notification(cib_t * cib, const char *callback, int enabled)
{
    ...
    if (cib->state != cib_disconnected) {
        ...
        rc = crm_ipc_send(native->ipc, notify_msg, NULL, cib->call_timeout);
        if(rc <= 0) {
